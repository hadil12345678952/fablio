...
import { nombreBlocs, blocsEnrichisPourEleve } from "@/lib/blocs/queries";
import { RenduParcoursFable } from "@/components/blocs/rendu-parcours-fable";
...
export default async function PageFableEleve({
  ...
  // ===== Nouveau moteur : rendu par blocs (avec fallback historique) =====
  const nbBlocs = await nombreBlocs(fable.id);
  const blocs = nbBlocs > 0 ? await blocsEnrichisPourEleve(fable.id, eleve.id) : null;

  // Texte lu par la synthèse vocale : sans les marques de mise en forme.
  const texteALire = blocs
    ? [
        fable.titre,
        ...blocs
          .filter((b) => b.type === "texte")
          .map((b) => String((b.contenu as { markdown?: string }).markdown ?? "")),
        fable.morale ? `Morale : ${fable.morale}` : "",
      ]
        .filter(Boolean)
        .join(". ")
        .replace(/\*\*/g, "")
        .replace(/\*/g, "")
    : [fable.titre, fable.texte, fable.morale ? `Morale : ${fable.morale}` : ""]
        .filter(Boolean)
        .join(". ")
        .replace(/\*\*/g, "")
        .replace(/\*/g, "");
    ...
    <div className="space-y-8">
      ...
      <div className="anim-apparition">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <h1 className="font-titre text-4xl font-bold tracking-tight text-balance">
            {fable.titre}
          </h1>
      ...
      {blocs ? (
        <>
          {/* ===== Rendu par blocs (ordre libre choisi par l'enseignant) ===== */}
          <RenduParcoursFable blocs={blocs} mode="eleve" lectureVocaleTexte={texteALire} />

          {/* Morale (métadonnée affichée en fin de parcours) */}
          {fable.morale && (
            <div className="rounded-3xl border-3 border-dashed border-ambre/50 bg-ambre/10 mx-auto max-w-2xl px-6 py-5">
        ...
        <>
          {/* ===== Fallback historique (fable sans blocs) ===== */}
